...
build_target() {
  ...
  while IFS='|' read -r board shield snippet cmake_args artifact_name; do
    if [ "$artifact_name" = "$target_name" ]; then
      ...
      # Execute build
      # $COMMAND /bin/bash -c "git config --global --add safe.directory '*' && west build '${build_args[@]}' -- '${cmake_args_array[@]}'"
...
init() {
  ...
  # $COMMAND /bin/bash -c "git config --global http.postBuffer 524288000 && git config --global --add safe.directory '*' && west config update.narrow true && west config update.fetch smart && west -v update"

  $COMMAND /bin/bash -c "west config update.narrow true && west config update.fetch smart && west -vv update"
